...
def MRR(y_true, y_pred):
    ...
    reciprocal_ranks = torch.zeros(y_true.shape[0], device=y_true.device)
    for i in range(y_true.shape[0]):
        relevant_items = (y_true_sorted[i] == 1).nonzero(as_tuple=True)[0]
        if relevant_items.numel() > 0:
            reciprocal_ranks[i] = 1.0 / (relevant_items[0].item() + 1)
...
class NormalizedDCG(Metric):
    ...
    def update(self, preds, target):
        ...
        # res_log2 = res_log2.scatter(1, indices, log_val)
